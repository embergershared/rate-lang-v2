...
subscription_key = os.environ["SPEECH_SERVICE_SUBSCRIPTION_KEY"]
region = os.environ["SPEECH_SERVICE_REGION"]
language = "en-US"
        ...
        return jsonify(
            {
                "code": 200,
                "storyid": id,
                "storynumelements": len(stories[id]),
                "story": stories[id],
            }
        )
...
def ackaud():
    ...
    pronAssessmentParamsJson = (
        '{"ReferenceText":"%s","GradingSystem":"HundredMark","Dimension":"Comprehensive","EnableMiscue":"True"}'
        % referenceText
    )
    ...
    # build request
    url = (
        "https://%s.stt.speech.microsoft.com/speech/recognition/conversation/cognitiveservices/v1?language=%s&usePipelineVersion=0"
        % (region, language)
    )
    headers = {
